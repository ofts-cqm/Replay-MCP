...
```text
plugins/replay-director-template/
├── plugin.json
├── mcp.json
...
│       └── test/
├── plugins/
│   ├── replay-director-template/        # authored Agent/Codex plugin files
│       ├── plugin.json
│       ├── mcp.json
│       ├── skills/
│       ├── assets/
│   └── replay-director/                 # generated, ignored installable plugin
│       ├── bin/                         # generated release bundle
│       └── protocol/                    # copied release contract
...
`packages/mcp-server` is an npm package. `plugins/replay-director-template` is
the authored plugin source, while `plugins/replay-director` is its generated
distribution; neither is an npm workspace package.
...
- released plugin archives should contain the exact tested JavaScript bundle
  under `plugins/replay-director/bin/`; the archive is built from the tracked
  `plugins/replay-director-template/`; and
...
Author the portable plugin template at `plugins/replay-director-template`, with
root `plugin.json`, root `mcp.json`, `skills/`, and `assets/`. Add a
...
`plugins/replay-director` is generated by the release build from
`plugins/replay-director-template`, `packages/mcp-server`, and
`protocol/bridge-v1`; it is ignored and is not a second source tree. A local
marketplace test must run `npm run package:plugin` before installing/copying the
plugin, because Codex loads the installed plugin copy rather than the workspace
source directory.
...
10. Build `plugins/replay-director` from the authored template, then add
    artistic and editor-specific skills only after the underlying tool behavior
    is verified.
